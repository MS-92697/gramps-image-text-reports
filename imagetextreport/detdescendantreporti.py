...
from gramps.gen.plug.docgen import (IndexMark, FontStyle, ParagraphStyle,
                                    TableStyle, TableCellStyle, TextDoc,
...
from gramps.gen.lib import Citation, Media, Note
#------------------------------------------------------------------------
#
# Constants
#
...
#
#------------------------------------------------------------------------
# TODO see what can be derived from standard report
# gramps.plugins.textreport.detdescendantreport.DetDescendantReport
# without carrying duplicate code all on the way
class DetailedDescendantReportI(Report):
    ...
    def write_images(self, media_list):
        ...
        cells = 1 # the GalleryTable has 3 cells, modified to 1 cell MS92697
        self.doc.start_row()
        ...
        while ( media_count < len(media_list) ):
            ...
            media_handle = media_ref.get_reference_handle()
            media: Media | None = self.database.get_media_from_handle(media_handle)
            if media is None:
    ...
    def write_media_notes(self, media: Media):
        note_handles = media.get_referenced_note_handles()
        for _, handle in note_handles:
